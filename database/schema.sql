...
create table public.rivalries (
  ...
  winner_id       uuid references public.profiles(id) on delete set null,  -- null while active
...
create table if not exists public.draft_sessions (
  ...
  total_picks     int not null,           -- total teams to draft
  pick_deadline   timestamptz,            -- when current picker's window expires
  created_at      timestamptz default now(),
  updated_at      timestamptz default now()
);
