...
### ⚠️ I18N RULES (MANDATORY)

- **NEVER hardcode user-facing strings** — every label, message, placeholder, error, or button text MUST use `t("key")` from `react-i18next`
- **Always import and use the hook**: `const { t } = useTranslation();`
- **Locale files are the single source of truth** for all copy:
  - `app/locales/en.json` — English
  - `app/locales/es.json` — Spanish
- **Before adding a new key**, check both locale files to avoid duplicates
- **Always add the key to BOTH locale files** at the same time — never leave a language missing
- **Key naming convention** — use dot-notation namespaced by screen/feature:
  - `"login.continue"`, `"profile.save_changes"`, `"reservation.confirm_booking"`
- **Never use the `defaultValue` fallback** as a substitute for adding the key properly to locale files
- **Yup validation messages** must also use `t("key")`, not hardcoded strings
